...
async def process_image(
    ...
    if result.final_image_path:
        response["final_image_url"] = f"/api/media/{Path(result.final_image_path).name}"
...
async def get_media(filename: str):
    """Retrieve a processed image or video."""
    media_path = storage.output_dir / filename
    if not media_path.exists():
        media_path = storage.temp_dir / filename
    if not media_path.exists():
    ...
    elif suffix in (".mp4", ".mov", ".avi", ".mkv", ".webm", ".wmv"):
        if suffix == ".mp4":
...
<body>
    <div class="container">
            ...
            <form id="uploadForm">
                <div class="upload-zone" id="dropZone">
                    ...
                    <div class="upload-text">
                        ...
                        <span id="fileInfo">Supports: JPG, PNG, WebP, MP4, MOV, AVI (images & videos)</span>
                    </div>
                    <input type="file" id="fileInput" accept="image/*,video/*" style="display:none">
                </div>
                <div class="preview-container" id="previewContainer">
                    <img id="previewImage" src="" alt="Preview" style="display:none">
                    <video id="previewVideo" controls style="display:none;max-width:100%;max-height:300px;border-radius:8px;margin:0 auto"></video>
                </div>
        ...
        <div class="card">
            <h2>📤 Multi-File Batch Upload</h2>
            <p style="color:#888;font-size:0.9rem;margin-bottom:1rem">
                Upload images/videos and paste their WhatsApp captions. They'll be saved to the batch folder
            ...
            <div class="upload-zone" id="multiDropZone" style="margin-bottom:1rem">
                ...
                <input type="file" id="multiFileInput" accept="image/*,video/*" multiple style="display:none">
            </div>
    ...
    <script>
        ...
        document.getElementById('uploadForm').addEventListener('submit', async (e) => {
            ...
            if (!file) { alert('Please select an image or video first!'); return; }
            ...
            try {
                ...
                if (result.final_image_url) {
                    const isVideo = result.final_image_url.match(/\.(mp4|mov|avi|mkv|webm|wmv)$/i);
                    if (isVideo) {
                        html += `<video src="${result.final_image_url}" class="result-image" controls></video>`;
                    } else {
            ...
            div.innerHTML = `
                <div class="pair-image">
                    ${isVideo
                        ...
                        : `<img src="${url}" alt="${file.name.replace(/["<>]/g, '')}" style="width:100%;height:100%;object-fit:cover">`
                    }
                </div>
        ...
        function updateUploadCounter() {
            ...
            uploadCounter.textContent = count === 0
                ? 'No media added yet'
                : count + ' file' + (count > 1 ? 's' : '') + ' ready to post';
        }
        ...
        document.getElementById('uploadAndStartBtn').addEventListener('click', async () => {
            ...
            btn.disabled = true;
            btn.textContent = '⏳ Uploading ' + validCount + ' file' + (validCount > 1 ? 's' : '') + '...';
            uploadStatus.style.display = 'block';
            uploadStatus.textContent = '📤 Uploading ' + validCount + ' file' + (validCount > 1 ? 's' : '') + '...';
